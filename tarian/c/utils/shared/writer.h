...
stain uint16_t write_str(uint8_t *buf, uint64_t *pos, unsigned long data_ptr, uint16_t n, enum memory mr){
    ...
    if (mr == USER){
        written_bytes = bpf_probe_read_user_str(&buf[SAFE_ACCESS(*pos)], n, (char *)data_ptr);
    } else {
        written_bytes = bpf_probe_read_kernel_str(&buf[SAFE_ACCESS(*pos)], n, (char *)data_ptr);
    }
...
#define MAX_CHARBUF_POINTERS 108
...
#pragma unroll
    for (; n < MAX_CHARBUF_POINTERS; ++n){
        ...
        if (!charbuf_pointer) break;
        
        char arr[28];
        bpf_probe_read_user_str(&arr, 28, (char *)charbuf_pointer);
        bpf_printk("Execve write ........ %d %s", n, arr);

        arg_len = bpf_probe_read_user_str(&buf[SAFE_ACCESS(curr_pos)], MAX_STRING_SIZE, (char *)charbuf_pointer);
        // write_str(buf, pos, charbuf_pointer, MAX_STRING_SIZE, USER);
        if(arg_len <= 0) break;
    ...
    // *pos += total_len & (MAX_STRING_SIZE - 1);
...
stain uint16_t write_byte_arr(uint8_t *buf, uint64_t *pos, unsigned long data_ptr, uint16_t n, enum memory mr){
    ...
    *pos += sizeof(uint16_t);
    ...
    if (mr == USER) {
        if(bpf_probe_read_user(&buf[SAFE_ACCESS(*pos)], n, (void *)data_ptr) != 0){
            return 0;
        }
    } else {
        if(bpf_probe_read_kernel(&buf[SAFE_ACCESS(*pos)], n, (void *)data_ptr) != 0){
            return 0;
        }
    }
