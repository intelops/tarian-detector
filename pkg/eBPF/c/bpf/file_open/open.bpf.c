...
BPF_RINGBUF_MAP(open_event_map);
...
int kprobe_open_entry(struct pt_regs *ctx) {
  ...
  // allocate space for an open_event_map in map.
  ed = BPF_RINGBUF_RESERVE(open_event_map, *ed);
  if (!ed) {
    return -1;
  }
  ...
  // pushes the information to ringbuf open_event_map mamp
