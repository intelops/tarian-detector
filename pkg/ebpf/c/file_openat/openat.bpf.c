...
// program attaches to sys_openat syscall
...
int kprobe_openat(struct pt_regs * ctx){
    ...
    // flags
    ed->flags = (int)PT_REGS_PARM3_CORE(ctx2);

    // filepath
    char *file = (char *)PT_REGS_PARM2_CORE(ctx2);
    bpf_probe_read_user_str(&ed->filename, sizeof(ed->filename), file);
